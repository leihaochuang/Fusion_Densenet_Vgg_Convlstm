...
import numpy as np
import mayavi.mlab
import utils_vitoimg.data_provider
...
def test_projection(pc, calib, img, save_to, filter_height, show, add):
    ...
    if show:
        show_pixels(coor=pixel, saveto=save_to+"vel2img_"+img[11:-4]+".png")

    # add pixels to image
    if add:
        add_pc_to_img(img_path=img, coor=pixel, saveto=save_to+img[11:-4]+'.png')


if __name__ == "__main__":
    for i in range(100):
        if i < 10:
            filename = "uu_00000" + str(i)
        else:
            filename = "uu_0000" + str(i)
        pc_path = "E:/dataset/kitti/data_road_velodyne/testing/velodyne/"+filename+".bin"
        calib_path = "E:/dataset/kitti/data_road/testing/calib/"+filename+".txt"
        image_path = "./data/img/"+filename+".png"
        test_projection(pc=pc_path, calib=calib_path, img=image_path, save_to='./result/', filter_height=None,
                        show=True, add=True)
        print("已转换完成图片数目："+' '+str(i+1))
