...
def output_result(model, test_loader, device):
    ...
    with torch.no_grad():
        for sample_batched in test_loader:
            ...
            data = torch.squeeze(data_image).cpu().numpy()
            ...
            data.save(fusion_config.save_path + "%s_data.png" % k)#red line on the original image
            img.save(fusion_config.save_path + "%s_pred.png" % k)#prediction result
...
def evaluate_model(model, test_loader, device, criterion):
    ...
    test_acc = 100. * int(correct) / (len(test_loader.dataset) * fusion_config.label_height * fusion_config.label_width)
...
if __name__ == '__main__':
    ...
    if args.model == 'Densenet-Vgg':
        test_loader = torch.utils.data.DataLoader(
            RoadSequenceDatasetList(file_path=fusion_config.image_test_path,
                                    file_path_Point_cloud_image=fusion_config.point_cloud_image_test_path,
                                    transforms=op_tranforms),
            batch_size=args.test_batch_size, shuffle=False, num_workers=1)
    else:
        test_loader = torch.utils.data.DataLoader(
            RoadSequenceDataset(file_path=fusion_config.image_test_path,
                                    transforms=op_tranforms),
            batch_size=args.test_batch_size, shuffle=False, num_workers=1)
    ...
    pretrained_dict = torch.load(fusion_config.pretrained_path)
